...
const TYPES_OUTPUT = path.resolve("types/env.ts");
const CLIENT_ENV_OUTPUT = path.resolve("config/client-env.ts");
...
function run() {
  const allKeys = ENV_FILES.flatMap(file => extractEnvKeys(path.resolve(file)));
