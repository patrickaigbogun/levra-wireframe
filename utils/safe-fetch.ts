import { Fetcher, SafeFetchOptions, SafeFetchResult } from "@/types/utils/fetcher";
...
export async function safeFetch<T extends {}>(
  ...
  try {
    ...
    if (response.success !== "ok" || response.data === null) {
      return {
        data: null,
        error: response.message || "Something went wrong and an error message was not provided.",
      };
    }
    ...
    return {
      data: null,
      error: err.message || "Unexpected error that was not handled and returned no response",
    };
  }
}
